...
#ifndef __PDUAL_CHANNEL_BWD_H__
#define __PDUAL_CHANNEL_BWD_H__
...
/* This class is a wrapper for Bwd when used for PDualChannelConstraints. It executes
...
class PDualChannelBwd : public Reachability{
public:
  ...
  virtual Result *reachability(Arg *arg) const{
    ...
    if(res->result == Reachability::REACHABLE){
      ...
      assert(dynamic_cast<PDualChannelConstraint::Common*>(static_cast<ExactBwd::Result*>(res)->common));
      Trace *old_trace = res->trace;
      res->trace = convert_trace(old_trace,
                                 static_cast<PDualChannelConstraint::Common*>(static_cast<ExactBwd::Result*>(res)->common));
      //delete old_trace;
    }
    return res;
  };
...
protected:
  virtual Trace *convert_trace(Trace *trace,PDualChannelConstraint::Common *common) const;
private:
