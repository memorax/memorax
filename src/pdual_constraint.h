...
#ifndef __PDUAL_CONSTRAINT_H__
#define __PDUAL_CONSTRAINT_H__
...
class PDualConstraint : public PDualChannelConstraint{
private:  
...
public:
  class Common : public PDualChannelConstraint::Common{
  public:
    Common(const Machine &m);
    virtual ~Common() {};
    virtual std::string to_string() const {
      return "<PDualConstraint::Common>";
    };
  ...
  private:
    ...
    std::vector<std::vector<Lang::NML>> nmls_by_proc;

    friend class PDualConstraint;
    friend class PDualTsoBwd;
  ...
  PDualConstraint(std::vector<int> pcs, const Common::MsgHdr &msg, Common &c);
  PDualConstraint(std::vector<int> pcs, Common &c);
  virtual PDualConstraint *clone() const { return new PDualConstraint(*this); }
...
private:
  ...
  struct pre_constr_t{
    pre_constr_t(PDualConstraint *sbc) : sbc(sbc), pop_back(false), written_nmls() {};
    pre_constr_t(PDualConstraint *sbc, bool pop_back, VecSet<Lang::NML> wn)
