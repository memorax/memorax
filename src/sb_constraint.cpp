    ...
    if(it->pop_back){
      ...
      if(it->sbc->channel.back().nmls == it->written_nmls){
        if(it->sbc->channel.size() == 1){
          ...
          std::vector<ChannelConstraint*> chcs = it->sbc->channel_pop_back();
          for(ChannelConstraint *chc : chcs){
            assert(dynamic_cast<SbConstraint*>(chc));
            SbConstraint *sbc = static_cast<SbConstraint*>(chc);
            if(move_p_to_last){
              sbc->cpointers[t.pid] = int(sbc->channel.size())-1;
            }
            if(sbc->ok_channel()){
              res.push_back(sbc);
            }else{
