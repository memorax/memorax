...
#ifndef __DUAL_CHANNEL_CONSTRAINT_H__
#define __DUAL_CHANNEL_CONSTRAINT_H__
...
/* Common base class for constraint classes using as DUAL-style channel. */
class DualChannelConstraint : public Constraint{
public:
    class Common;
private:
...
protected:
    /* The class of DUAL channel messages. */
    class Msg{
    public:
...
public:
    ...
    protected:
        ...
        friend class DualChannelConstraint;
        friend class DualChannelBwd;
    };
    ...
    DualChannelConstraint(std::vector<int> pcs, const Common::MsgHdr &msg, Common &c);
    DualChannelConstraint(std::vector<int> pcs, Common &c);
    /* Returns a copy of the constraint */
    virtual DualChannelConstraint *clone() const = 0;
     ...
     */
    std::vector<std::vector<MsgCharacterization>> characterize_channels() const;
...
protected:
    ...
    bool inline possibly_holds(const Store &reg_store, int pid, const Lang::BExpr<int> &b) const{
        throw new std::logic_error("DualChannelConstraint::possibly_holds: Not implemented");
    };
...
private:
    Comparison entailment_compare_impl(const DualChannelConstraint &sbc) const;
    Common &common;
     ...
     * cmp.
     */
    virtual Constraint::Comparison entailment_compare_channels(const DualChannelConstraint &sbc, Constraint::Comparison cmp) const;
