...
const bool PDualChannelContainer::print_every_state_on_clear = false;
const bool PDualChannelContainer::use_genealogy = false;

PDualChannelContainer::PDualChannelContainer(){
  last_popped.first = 0;
...
void PDualChannelContainer::insert_root(Constraint *r){
  insert(new CWrapper(static_cast<PDualChannelConstraint*>(r)));
};

void PDualChannelContainer::insert(Constraint *p, const Machine::PTransition *t, Constraint *c){
  CWrapper *pcw = get_cwrapper(static_cast<PDualChannelConstraint*>(p));
  CWrapper *cw = new CWrapper(static_cast<PDualChannelConstraint*>(c), pcw, t);
//  Log::extreme << "before insert\n";
  if(insert(cw)){
    if(use_genealogy){
...
bool PDualChannelContainer::insert(CWrapper *cw){
...
void PDualChannelContainer::invalidate(CWrapper *cw, std::vector<CWrapper*> *Fv){
  if(Fv == 0){
    Fv = &get_F_set(cw);
  }
#ifndef NDEBUG
...
Constraint *PDualChannelContainer::pop(){
  if(q_size == 0){
    return 0;
  }else{
    CWrapper *cw = Q.pop();
...
Trace *PDualChannelContainer::clear_and_get_trace(Constraint *c){
  Trace *t = new Trace(c);
  CWrapper *cw = get_cwrapper(static_cast<PDualChannelConstraint*>(c));
  cw->sbc = 0;
  while(cw->parent){
...
void PDualChannelContainer::clear(){
...
std::vector<PDualChannelContainer::CWrapper*> &PDualChannelContainer::get_F_set(CWrapper *cw) {
  return F[1][1];
}

void PDualChannelContainer::visit_F(std::function<void(std::vector<CWrapper*>&)> f){
  for(auto &FPerPcs : F){
