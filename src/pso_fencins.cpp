...
namespace PsoFencins{
  ...
  std::list<cycle_t> find_cycles(const Trace &trace){
    ...
    for(int t = 1; t <= trace.size(); t++){
      ...
      case Lang::FENCE: case Lang::SSFENCE:
        // Do nothing
        break;
  ...
  void FenceSet::print(Log::redirection_stream &text, Log::redirection_stream &json) const {
    if (slocks.empty() && mlocks.empty()) {
      ...
      for (const Machine::PTransition &trans : slocks) {
        ...
        json << "json: {\"action\":\"Link Fence\", \"pos\":" << trans.instruction.get_pos().to_json() << "}\n";;
      }
  ...
  std::set<Sync*> FenceSet::to_sync_set() const{
    std::set<Sync*> S;
    for (const auto &slock : slocks) S.insert(new PsoSlockSync(slock));
    for (const auto &mlock : mlocks) S.insert(new TsoLockSync(mlock));
    return S;
  };
    ...
    if (slocks.count(baset) == 0 && mlocks.count(baset) == 0) {
      ...
      for (Automaton::Transition *mt : states[t.source].fwd_transitions) {
        if (mt->target == t.target && mt->instruction == t.instruction) {
          ...
          atomized_machine.automata[t.pid].add_transition({fence, Lang::Stmt<int>::ss_fence(), target});
          mt->target = fence;
      ...
      for (Automaton::Transition *mt : states[t.source].fwd_transitions) {
        if (mt->target == t.target && mt->instruction == t.instruction) {
          if (slocks.count(baset)) {
            ...
            assert((*states[mt->target].fwd_transitions.cbegin())->instruction.get_type() == Lang::SSFENCE);
            (*states[mt->target].fwd_transitions.begin())->instruction = Lang::Stmt<int>::full_fence();
          } else {
            /* We create a new state as above. */
            int fence = int(states.size());
            int target = mt->target;
            atomized_machine.automata[t.pid].add_transition({fence, Lang::Stmt<int>::full_fence(), target});
            mt->target = fence;
            states[target].bwd_transitions.erase(mt);
            states[fence].bwd_transitions.insert(mt);
          }
