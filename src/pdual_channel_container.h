...
#ifndef __PDUAL_CHANNEL_CONTAINER__
#define __PDUAL_CHANNEL_CONTAINER__
...
/* A constraint container meant for PDualChannelConstraints. Uses
 * PDualChannelConstraint::entailment_compare for comparison and entailment upon
 * insertion.
 */
class PDualChannelContainer : public ConstraintContainer{
public:
  PDualChannelContainer();
  virtual ~PDualChannelContainer();
  ...
  virtual void clear();
protected:
  /* Keeps a PDualChannelConstraint and some extra information about it. */
  struct CWrapper{
    CWrapper(PDualChannelConstraint *sbc, CWrapper *parent = 0, const Machine::PTransition *pt = 0)
     ...
     * PDualChannelContainer. (Most likely in
...
private:
   ...
   */
    std::map<int, std::map<int,std::vector<CWrapper *> > >  F;
   ...
   * CWrapper in F.
   */
  std::map<PDualChannelConstraint*,CWrapper*> ptr_to_F;

  /* Caches (sbc,cw) for the last constraint sbc that was popped, and
   * cw == ptr_to_F[sbc].
   */
  std::pair<PDualChannelConstraint*,CWrapper*> last_popped;

  /* Returns ptr_to_F[sbc]. Uses the cache last_popped if possible.
   */
  CWrapper *get_cwrapper(PDualChannelConstraint *sbc) const{
  ...
  struct stats_t{
    ...
    void print(){
      Log::debug << " ===============================\n"
                 << " = PDualChannelContainer statistics =\n"
                 << " ===============================\n"
