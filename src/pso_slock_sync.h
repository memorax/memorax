...
#ifndef __PSO_SLOCK_SYNC_H__
#define __PSO_SLOCK_SYNC_H__
...
/* A PsoSlockSync synchronization is synchronization by changing a
 * specific write (write: x := e) into a store-store locked write
 * (slocked write: x := e).
 */
class PsoSlockSync : public Sync{
public:
  /* Construct synchronization that replaces w with w_pid:
   * (w_src,slocked write: w_x := w_e ,w_tgt).
   *
   * Pre: w is w_pid: (w_src,write: w_x := w_e ,w_tgt) for some w_pid,
   * w_src, w_x, w_e, w_tgt.
   */
  PsoSlockSync(const Machine::PTransition &w);
  virtual ~PsoSlockSync() {};

  class InsInfo : public Sync::InsInfo{
  public:
    InsInfo(const PsoSlockSync *creator_copy);
    InsInfo(const InsInfo &) = default;
    InsInfo &operator=(const InsInfo &) = delete;
    virtual ~InsInfo() {};

    /* When a PsoSlockSync is inserted into a Machine m, creating the
